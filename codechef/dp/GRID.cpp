...
vector<vector<int>> computeSuffixSum(const vector<vector<int>> &matrix) {
  int n = matrix.size();
  int m = matrix[0].size();

  vector<vector<int>> suffixSum(n, vector<int>(m, 0));

  for (int i = n - 1; i >= 0; --i) {
    for (int j = m - 1; j >= 0; --j) {
      suffixSum[i][j] = matrix[i][j];
      ...
      if (i + 1 < n && j + 1 < m)
        suffixSum[i][j] -= suffixSum[i + 1][j + 1];
    }
  }
...
void solve() {
  ...
  auto suffG = computeSuffixSum(grid);
