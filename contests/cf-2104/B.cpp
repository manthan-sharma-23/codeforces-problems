...
class SegmentTree {
  ...
  void build(int node, int start, int end, vector<ll> &a) {
    if (start == end) {
      tree[node] = a[start];
    } else {
      int mid = (start + end) / 2;
...
public:
  ...
  int getMax(int l, int r) { return query(1, 0, n - 1, l, r); }
};
...
void solve() {
  ...
  for (int k = 1; k <= n; k++) {
    ll currWin = p[n] - p[n - k], shortWin = p[n] - p[n - k + 1];

    ll rangeMax = segT.getMax(1, n - k);

    cout << max(currWin, rangeMax + shortWin) << " ";
  }
  cout << endl;
}
