...
void solve() {
  ...
  if (toFill) {
    bool findPlaceToInsert1 = false;
    each(it, sbs) {
      if (it > 1) {
        findPlaceToInsert1 = true;
        break;
      }
    }

    if (findPlaceToInsert1) {
      sum += 2;
    } else {
      ll smallestBitpresent = 1;
      while (!sbs.count(smallestBitpresent)) {
        smallestBitpresent <<= 1;
      }
      smallestBitpresent <<= 1;

      sum += smallestBitpresent * 2;
    }
  }
