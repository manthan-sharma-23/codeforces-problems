...
void solve() {
  ...
  vector<vll> dp(N + 1, vll(K + 1, INT_MAX));

  int currMax = 0;
  for (int i = 1; i <= N; i++) {
    currMax = max(currMax, a[i]);
    ll space = (i * currMax) - pref[i];
    dp[i][0] = space;
  }
  ...
  for (int i = 1; i <= N; i++) {
    for (int k = 1; k <= K; k++) {
      int currMax = a[i];
      for (int c = i - 1; c >= 0; c--) {
        int sum = pref[i] - pref[c];
        int len = i - c;
        ll space = (1LL * len * currMax) - sum;
        if (dp[c][k - 1] != LLONG_MAX)
          dp[i][k] = min(dp[i][k], dp[c][k - 1] + space);
        currMax = max(currMax, a[c]);
      }
    }
  }
